...
Public NotInheritable Class Packer
    ...
    Private Const VERSION As Byte = 3
    ...
    Public Shared Sub Pack(dayMapFolder As String, outputUMDPath As String,
        ...
        For Each zDir In Directory.EnumerateDirectories(tilesRoot)
            ...
            For Each xDir In Directory.EnumerateDirectories(zDir)
                If File.Exists(Path.Combine(xDir, "DATA.CNT")) Then
        ...
        Dim totalCount = 0
        Dim probeContainerPath As String = Nothing
        Dim probeKey As String = Nothing
        ...
        Parallel.ForEach(xDirEntries, scanParallelOptions,
            Sub(item)
                ...
                Dim containerPath = Path.Combine(item.XDir, "DATA.CNT")
                Using cReader = TileDataContainer.OpenRead(containerPath)
                    For Each key In cReader.Keys
                        Dim yVal As Integer
                        If Not Integer.TryParse(key, yVal) Then Continue For

                        If probeContainerPath Is Nothing Then
                            SyncLock probeLock
                                If probeContainerPath Is Nothing Then
                                    probeContainerPath = containerPath
                                    probeKey = key
                                End If
                            End SyncLock
                        End If
        ...
        If totalCount = 0 Then
            Throw New InvalidOperationException("No DATA.CNT-Tiles found under " & tilesRoot)
        End If
        ...
        Using probeReader = TileDataContainer.OpenRead(probeContainerPath)
            Dim probeBytes As Byte() = Nothing
            If Not probeReader.TryGet(probeKey, probeBytes) Then
                Throw New InvalidOperationException($"Probe tile '{probeKey}' missing from {probeContainerPath}")
            End If
            Using probeMs As New MemoryStream(probeBytes)
                Dim probeInfo = SixLabors.ImageSharp.Image.Identify(probeMs)
                If probeInfo.Width <> probeInfo.Height Then
                    Throw New InvalidOperationException($"Tile is not square: {probeContainerPath}::{probeKey}")
                End If
        ...
        Using fs As New FileStream(tmpPath, FileMode.Create, FileAccess.ReadWrite,
            ...
            Using bw As New BinaryWriter(fs, Encoding.UTF8, leaveOpen:=True)
                ...
                Dim headerStartPos = fs.Position

                bw.Write(FormatVersion.MagicBytes(FormatVersion.CurrentMagic()))
                bw.Write(VERSION)
                ...
                Dim usedSoFar = fs.Position - headerStartPos
                Dim paddingLength = HEADER_SIZE - usedSoFar
                If paddingLength < 0 Then
                    Throw New InvalidOperationException(
                        $"Header-Felder ({usedSoFar} Bytes) überschreiten HEADER_SIZE ({HEADER_SIZE}). " &
                        "HEADER_SIZE muss erhöht werden (und im Reader synchron gehalten werden).")
                End If
                If paddingLength > 0 Then
                    bw.Write(New Byte(CInt(paddingLength) - 1) {})
                End If
                                    ...
                                    Sub(item)
                                        ...
                                        Using cReader = TileDataContainer.OpenRead(containerPath)
                                            For Each key In cReader.Keys
                                                ...
                                                If Not cReader.TryGet(key, pngBytes) Then Continue For
        ...
        If File.Exists(outputUMDPath) Then File.Delete(outputUMDPath)
        File.Move(tmpPath, outputUMDPath)

        Dim sizeMB = New FileInfo(outputUMDPath).Length / (1024.0 * 1024.0)
        progress?.Invoke($"Finished! {outputUMDPath} – {sizeMB:F1} MB, TileSize={tileSize}, Mode=GZip(PNG)")
    End Sub
