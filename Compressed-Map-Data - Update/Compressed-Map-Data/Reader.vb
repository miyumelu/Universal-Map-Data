...
Public NotInheritable Class Reader
    ...
    Public Shared Function TryCreate(Optional explicitPath As String = Nothing,
        ...
        For Each path In candidates
            Dim reader = TryLoad(path, cacheCapacity)
            If reader IsNot Nothing Then Return reader
        Next
        Return Nothing
    End Function

    Private Shared Function TryLoad(cmdPath As String, cacheCapacity As Integer) As Reader
        ...
        Try
            Dim fileLen = New FileInfo(cmdPath).Length
            mmf = MemoryMappedFile.CreateFromFile(cmdPath, FileMode.Open, Nothing, 0, MemoryMappedFileAccess.Read)
            ...
            If Encoding.ASCII.GetString(magicBytes) <> MAGIC Then
            ...
            Return New Reader(info, indexOff, tileCount, entrySize,
        ...
        Try
            ...
            ElseIf _compressionMode = 2 Then
                Using ms As New MemoryStream(compressed, 0, entry.CompLen, writable:=False)
                    Using img As Image(Of Bgra32) = SixLabors.ImageSharp.Image.Load(Of Bgra32)(ms)
                ...
                Using cms As New MemoryStream(compressed, 0, entry.CompLen, writable:=False)
                    Using gs As New GZipStream(cms, CompressionMode.Decompress)
                        Using pngMs As New MemoryStream()
                            gs.CopyTo(pngMs)
                            pngMs.Position = 0
                            Using img As Image(Of Bgra32) = SixLabors.ImageSharp.Image.Load(Of Bgra32)(pngMs)
                                img.CopyPixelDataTo(raw)
                            End Using
                        End Using
                    End Using
